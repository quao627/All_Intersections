...
def get_lane_data(oneway, lanes, lanes_forward, lanes_both_ways, lanes_backward):
    if oneway == "yes":
        ...
        if lanes == lanes:
            lanes = int(lanes)
            lane_data = [0, 0, 0]
            if lanes_both_ways == 1:
                lane_data[1] = 1
            else:
                lane_data[1] = 0
            if lanes_forward == lanes_forward:
                lane_data[0] = int(lanes_forward)
                lane_data[2] = lanes - lane_data[0] - lane_data[1]
            elif lanes_backward == lanes_backward:
                lane_data[2] = int(lanes_backward)
                lane_data[0] = lanes - lane_data[1] - lane_data[2]
            else:
                if lanes % 2 == 0:
                    lane_data[0], lane_data[2] = lanes // 2, lanes // 2
                else:
...
def find_length_geometry(G, u, v, node_dict_m):
    def get_angle(u, v, node_dict_m):
        x1, y1 = node_dict_m[u]
        x2, y2 = node_dict_m[v]
        ...
        if y2 < y1:
            theta = 360 - theta
        return theta
    for head, tail, a in G.edges(data=True):
        nodes = a["nodes"]
        if u in nodes and v in nodes:
            theta = get_angle(u, v, node_dict_m)
            other_end = head if u==tail else tail
            length = a["length"]
            geometry = list(a["geometry"].coords)
            if len(G[other_end]) <= 3:
                for node in G[other_end]:
                    if abs(theta - get_angle(other_end, node, node_dict_m)) < 20:
                        length += G[other_end][node][0]["length"]
                        geometry.extend(list(G[other_end][node][0]["geometry"].coords))
            break
