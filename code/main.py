...
def get_data(city, state, out_path):
        ...
        for index, node in enumerate(bi_nodes):
            if node in G.nodes():
                ...
                if len(neighbors) == 2:
                    ...
                    G[neighbors[0]][neighbors[1]][k]["geometry"] = shapely.ops.linemerge(
                        [G[node][neighbors[0]][0]["geometry"], G[node][neighbors[1]][0]["geometry"]])
                    G.remove_node(node)
    ...
    groups = find_clusters(signals, threshold=50)
...
def get_intersection(roads, G_nodes, node_dict_m, G, df_group, restrictions):
    ...
    for group_index in range(len(df_group)):
        ...
        for (u, v, a) in edges:
            if u not in intersection_nodes:
                u, v = v, u
            a["length"], a["geometry"] = find_length_geometry(G, u, v, node_dict_m)
...
def fill_speed_gradient(intersection_dict, path="../data/salt-lake-city/slc_new.pkl"):
...
def get_satellite_image(intersection_dict, out_path):
    for index, intersection in intersection_dict.items():
        print(f"Saving image for intersection {index}")
        lat, lon = intersection["centroid"]
        get_sat_image(lat, lon, zoom=18, output=f"{out_path}/img{index}.jpeg")
...
if __name__ == "__main__":
    ...
    # if os.path.exists(f"{out_path}/intersections.pkl"):
    #     intersection = pickle.load(open(f"{out_path}/intersections.pkl", 'rb'))
    ...
    #                  data["restrictions"])
    #     with open(f"{out_path}/intersections.pkl", 'wb') as f:
    #         pickle.dump(intersection, f, protocol=pickle.HIGHEST_PROTOCOL)
    # get_satellite_image(intersection, out_path)
    intersection = pickle.load(open(f"../data/{'-'.join(city.lower().split(' '))}/intersections.pkl", "rb"))
